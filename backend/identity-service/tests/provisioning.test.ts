...
describe('AC-02 — slug duplicado', () => {
  it('devolve 409 ERR_IDENT_004 com sugestões e não cria nada no Clerk', async () => {
    ...
    await callApi({ method: 'POST', url: '/v1/tenants', clerkUserId: first, payload: validPayload })
...
describe('AC-03 — falha parcial no provisionamento', () => {
  ...
  it('o job de retry retoma o provisionamento de forma idempotente', async () => {
    ...
    const { runProvisioningRetryOnce } = await import('../src/modules/tenants/provisioning-retry.js')
...
describe('disponibilidade de slug (etapa 1 do wizard)', () => {
  it('classifica slug livre, ocupado, reservado e inválido', async () => {
    const clerkUserId = givenClerkUser('slug@petshop.test')
    await callApi({ method: 'POST', url: '/v1/tenants', clerkUserId, payload: validPayload })
...
describe('autenticação de serviço', () => {
  it('recusa requisição sem a assinatura do gateway', async () => {
    const { getApp } = await import('./harness.js')
  ...
  it('deixa /health passar sem assinatura', async () => {
    const { getApp } = await import('./harness.js')
    const app = await getApp()
