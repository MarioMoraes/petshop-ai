...
const ADMIN_PERMISSIONS: PermissionKey[] = ['team:read', 'team:invite', 'team:remove']

async function givenTenant(slug: string, plan = 'STARTER'): Promise<Admin> {
...
async function invite(admin: Admin, email: string, role = 'GROOMER') {
  return callApi({
    method: 'POST',
    url: '/v1/invitations',
    payload: { email, role },
...
describe('Aceite', () => {
  ...
  it('recusa token inexistente sem revelar nada', async () => {
    ...
    const response = await callApi({
      ...
      clerkUserId: givenClerkUser('ninguem@exemplo.com'),
    })
  ...
  it('recusa convite revogado', async () => {
    ...
    const response = await callApi({
      ...
      url: '/v1/invitations/accept',
      payload: { token: tokenOf(body.inviteUrl) },
      clerkUserId: givenClerkUser('tosador@exemplo.com'),
    })
...
describe('Reenvio e listagem', () => {
  ...
  it('lista os pendentes e esconde os já aceitos', async () => {
    ...
    await callApi({
      ...
      clerkUserId: givenClerkUser('aceito@exemplo.com'),
    })
  ...
  it('a prévia pública mascara o e-mail de quem foi convidado', async () => {
    ...
    const response = await callApi({
      ...
      clerkUserId: givenClerkUser('quemquer@exemplo.com'),
    })
