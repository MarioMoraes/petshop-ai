...
describe('AC-02 — o vínculo nasce', () => {
  ...
  it('recusa o desafio pedido por outra conta do Clerk', async () => {
    ...
    // O intruso tem conta no Clerk e espelho local, e nenhuma ficha neste petshop.
    await givenUser('intruso', 'user_intruso')
    const response = await callApi({
...
describe('GET /portal/v1/tenant — a tela de login', () => {
  it('devolve a identidade visual sem exigir tutor', async () => {
    // Sem token nenhum: a tela de login é a única rota do Portal que responde sem sessão.
