...
/** A terça-feira equivalente, para as séries semanais que precisam ser todas futuras. */
function tercaDistante(): Date {
  const dia = new Date()
  dia.setUTCHours(9, 0, 0, 0)
  dia.setUTCDate(dia.getUTCDate() + 35)
  // 2 = terça-feira em `getUTCDay()`.
  dia.setUTCDate(dia.getUTCDate() + ((2 - dia.getUTCDay() + 7) % 7))
  return dia
}
...
describe('MOD-AGENDA-05 — recorrência', () => {
  ...
  /**
   * **A mesma armadilha do `quintaDistante`, e ela detonou nesta linha.**
   *
   * Os outros casos de recorrência podem partir de uma data cravada porque só contam o
   * que `expandOccurrences` gera, e isso não depende de hoje. Este conta o que
   * `endRecurrence` **cancela**, e o filtro dele é `startsAt > agora` — cada semana que
   * passa empurra mais uma ocorrência de 2026-09-01 para o passado e derruba a
   * asserção. Partindo de uma terça futura, as doze ocorrências são futuras sempre, e a
   * única que sobra fora do cancelamento é a que o teste concluiu de propósito.
   */
