...
/**
 * O prefixo do Portal, e a pergunta que o `app.ts` faz antes de resolver a sessão.
 *
 * Morava no `proxy.ts`, que existia para saber a que serviço mandar cada rota. Com a fatia
 * 11 não há mais para onde mandar, e o que sobrou da pergunta é a única parte que sempre
 * importou: **o Portal resolve a sessão de outro jeito**. Ela não sai do token — o tutor não
 * tem Organization no Clerk —, sai do host que o Next serviu.
 *
 * O prefixo próprio, e não um ramo de `/v1`, é a decisão de segurança do módulo (AC-04 de
 * MOD-PORTAL-11): a superfície do cliente final tem rate limit e resolução de sessão
 * separados, e **nenhum papel `TUTOR` alcança o `/v1` administrativo**. No dia em que as
 * duas dividissem prefixo, uma rota nova do Admin nasceria ao alcance de quem tem só `_own`
 * sem que ninguém percebesse.
 */
const PORTAL_PREFIX = '/portal/v1'
