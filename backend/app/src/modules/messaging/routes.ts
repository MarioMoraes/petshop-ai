...
import { badWebhook, invalid, notFound, unauthorized } from './errors.js'
...
const REQUIRE_SEND = {
  preHandler: requirePermission('crm:send', 'Você não tem permissão para disparar mensagens'),
}
...
export async function registerMessagingRoutes(app: FastifyInstance): Promise<void> {
  ...
  /**
   * A porta de entrada do envio **por HTTP**. Responde **200** quando o `dedupeKey` já
   * existia e **202** quando enfileirou — a diferença importa para quem reprocessa evento
   * e quer saber se causou algo.
   *
   * **A exceção de chamada de serviço caiu na fatia 11, e ela tinha virado um buraco.**
   * A regra era "contexto sem `userId` passa; com `userId`, exige `crm:send`", e existia
   * porque os outros serviços chegavam aqui pela porta interna assinada, sem usuário. Com a
   * porta interna extinta, quem chega sem `userId` deixou de ser um serviço e passou a ser
   * **uma pessoa cujo espelho local ainda não existe** — o estado normal do primeiro acesso.
   * Mantê-la deixaria essa pessoa disparar mensagem em nome do tenant sem permissão nenhuma.
   *
   * Quem enfileira sem ser gente continua existindo: as automações do MOD-CRM e o Portal.
   * As duas chamam `enqueueMessage` direto, por porta de módulo, e nem passam por aqui.
   */
  app.post('/v1/messages', REQUIRE_SEND, async (request, reply) => {
...
export async function registerEmailWebhookRoutes(app: FastifyInstance): Promise<void> {
  await app.register(async (scope) => {
    ...
    scope.post('/internal/v1/email/webhook', async (request, reply) => {
      ...
      /**
       * 401 sem processar nada e sem revelar se a mensagem existe (AC-03) — **e uma linha
       * em `security_events`** (AC-02 de MOD-SEC-07).
       *
       * O emissor de `WEBHOOK_SIGNATURE_INVALID` mudou de casa na fatia 11, e voltou para
       * onde o nome do tipo sempre disse que ele estava: "assinatura inválida em chamada
       * **externa**". Até então quem o emitia era a porta interna do processo, que
       * verificava o HMAC de serviço a serviço — ela era o único emissor, e sumiu com a
       * consolidação. Esta é a superfície que o `Caddyfile` publica, e a única do backend
       * onde uma assinatura de fora é conferida.
       *
       * `tenantId` é `null` porque a assinatura era justamente o que diria de quem se
       * trata. O `await` importa: uma requisição recusada responde em milissegundos, e um
       * registro disparado sem espera perderia a corrida com o fim do processo numa réplica
       * encerrando.
       */
      if (!valid) {
        await recordSecurityEvent({
          ...
          targetEntity: 'email_webhook',
          targetId: (request.headers['svix-id'] as string | undefined) ?? null,
          ipAddress: request.ip,
