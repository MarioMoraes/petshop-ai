...
import { methodNotEnabled } from './errors.js'
import {
  CACHE_KEYS,
  CACHE_TTL_SECONDS,
  cacheGet,
 ...
 * `tenant_settings` desde o MOD-AGENDA, e é `modules/scheduling` quem calcula a taxa com
...
export async function getSettings(actor: ActorContext): Promise<BillingSettings> {
  const key = CACHE_KEYS.ledgerSettings(actor.tenantId)
  ...
  await cacheSet(key, settings, CACHE_TTL_SECONDS.ledgerSettings)
  return settings
}
...
export async function updateSettings(
  ...
  await invalidateLedgerSettings(actor.tenantId)
  return result
}
