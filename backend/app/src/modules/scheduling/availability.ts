...
/**
 * A grade de horários de um pedido, do começo ao fim (MOD-AGENDA-11).
 *
 * **A duração é calculada para *este* pet antes de varrer a agenda**: porte e pelagem
 * mudam o tamanho do buraco necessário, e disponibilidade sem pet seria uma aproximação
 * que a criação depois recusaria.
 *
 * Estava inline no handler de `GET /v1/availability`. Saiu de lá na fatia 11, quando o
 * Portal passou a precisar da mesma grade sem passar por HTTP: `readAvailability` do
 * MOD-PORTAL mostra ao tutor exatamente o que o balcão vê, e duas cópias desta sequência
 * divergiriam no dia em que uma delas ganhasse uma regra.
 */
export async function resolveAvailability(
  tenantId: string,
  query: z.output<typeof AvailabilityQuerySchema>,
