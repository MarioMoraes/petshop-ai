...
import { asRole, getApp, ownerPrisma, serviceHeaders, type Caller } from '../harness.js'
...
/**
 * Chamada de serviço: sem `userId`, e **pela porta interna**.
 *
 * É o que a rota de enfileiramento aceita sem exigir `crm:send`, e continua sendo o
 * contexto assinado — só que agora quem o reconhece é o `app.ts` do backend, e não a
 * porta de um serviço separado. É o único teste que exercita essa porta.
 */
export async function callAsService(fixture: TenantFixture, options: {
  ...
  return instance.inject({
    ...
    url: options.url,
    headers: serviceHeaders({ clerkUserId: 'svc_crm-automation', tenantId: fixture.tenantId }),
