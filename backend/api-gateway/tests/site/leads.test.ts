...
describe('fila (MOD-SITE-09)', () => {
  ...
  it('a fila devolve o telefone em claro para a equipe', async () => {
    ...
    const items = (
      await callApi({ ...(await asReceptionist(fixture)), method: 'GET', url: '/v1/site/leads' })
    ).json().items
  ...
  /**
   * RN-10: ver o contato e criar a ficha são permissões diferentes.
   *
   * A matriz padrão dá as duas à recepção, então o cenário precisa de um tenant que
   * tenha revogado `tutor:create` dela — o que `tenant_role_overrides` permite. É o
   * gate valendo sob uma configuração real, e não sob uma lista de permissões
  ...
  it('converter exige tutor:create além de ver a fila', async () => {
    ...
    const lead = await withTenant(fixture.tenantId, (tx) => tx.siteLead.findFirstOrThrow())
    const tutorId = await givenTutor(fixture, 'outro-hash')
    await revokePermission(fixture, 'RECEPTIONIST', 'tutor:create')
