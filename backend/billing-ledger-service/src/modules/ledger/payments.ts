...
import { alreadyReversed, invalidAllocation, notFound } from '../../lib/errors.js'
import { publishEvent } from '../../lib/events.js'
import { recordMetric } from '../../lib/logger.js'
import type { ActorContext } from './actor.js'
import { tenantOptions } from './actor.js'
import { lockAccount, openAccount, postEntry } from './accounts.js'
import {
  applyAllocations,
  lockDebit,
  lockOpenDebits,
...
export async function recordPayment(actor: ActorContext, input: CreatePaymentInput) {
  ...
  const result = await withTenant(
    ...
    async (tx) => {
      ...
      const cipher =
        input.notes || input.proofUrl ? await openCipher(tx, actor.tenantId) : null
      ...
      await confirmIdempotency(tx, actor.tenantId, ENDPOINT, input.idempotencyKey, payment.id)
      ...
      return { repeated: false as const, paymentId: payment.id, entry, manual, receipt }
    },
    tenantOptions(actor),
  )
