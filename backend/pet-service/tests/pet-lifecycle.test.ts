...
import { setSchedulingPort } from '../src/lib/scheduling.js'
import {
  asAdmin,
  callApi,
  catalogIds,
...
describe('MOD-PET-07 — histórico de peso', () => {
  ...
  it('o banhista pesa sem poder editar o cadastro', async () => {
    ...
    const caller = {
      ...
      userId: tenant.userId,
      tenantId: tenant.tenantId,
      role: 'BATHER' as const,
    }

    const weighed = await callApi({
      ...caller,
      permissions: ['pet:read', 'pet:weigh'],
      method: 'POST',
      url: `/v1/pets/${petId}/weights`,
      payload: { weightKg: 12 },
    })
...
describe('MOD-PET-08 — óbito', () => {
  ...
  it('a recepção registra o óbito, mas só o administrador reverte', async () => {
    ...
    const receptionist = {
      ...
      permissions: ['pet:read', 'pet:update', 'pet:weigh'] as const,
    }
