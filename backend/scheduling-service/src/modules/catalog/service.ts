...
import { invalid, notFound, serviceInUse } from '../../lib/errors.js'
...
export async function createService(actor: ActorContext, input: CreateServiceInput) {
  ...
  await invalidateCatalog(actor.tenantId, created.professionals.map((link) => link.professionalId))
...
export async function updateService(
  ...
  await invalidateCatalog(actor.tenantId, updated.professionals.map((link) => link.professionalId))
  await publishEvent('agenda.servico.alterado', {
    tenantId: actor.tenantId,
    serviceId,
...
export async function deleteService(actor: ActorContext, serviceId: string) {
  ...
  await invalidateCatalog(actor.tenantId)
}
