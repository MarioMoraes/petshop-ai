...
function post(path: string, payload: unknown, caller?: CallerOptions) {
  return callApi({
...
function get(path: string, caller?: CallerOptions) {
...
describe('MOD-PRONT-03 — alergias e restrições', () => {
  ...
  it('a recepção registra alergia, mas não a desativa', async () => {
    const receptionist: CallerOptions = {
      clerkUserId: tenant.clerkUserId,
      userId: tenant.userId,
      tenantId: tenant.tenantId,
      role: 'RECEPTIONIST',
      permissions: ['record:read_alerts', 'record:write_alerts', 'record:write_notes'],
    }
...
describe('MOD-PRONT-05 — alertas médicos', () => {
  ...
  it('a recepção não altera alerta médico', async () => {
    ...
    const response = await callApi({
      ...
      permissions: ['record:read_alerts', 'record:write_alerts'] as const,
      method: 'PATCH',
      url: `/v1/pets/${petId}/medical-alerts/${created.json().id}`,
...
describe('agregação de alertas', () => {
  ...
  it('o motorista vê os alertas — ele abre a caixa de transporte', async () => {
    ...
    const response = await get('/alerts', {
      ...
      tenantId: tenant.tenantId,
      role: 'DRIVER',
      permissions: ['record:read_alerts'],
    } satisfies CallerOptions)
