...
export async function carregarPendencias(me: MeResponse): Promise<ContagemPendencias> {
  ...
  const [
    ...
    /*
     * O plano antes da permissão, e os dois antes da chamada: sem o recurso a rota
     * responderia 402 em toda navegação, e o `catch` esconderia um erro que não é erro.
     * `inventory:read` é a permissão da lista para onde as três linhas apontam.
     */
