...
export function Lista({ inicial }: { inicial: PortalMessagesResponse }) {
  ...
  if (mensagens.length === 0) {
    return (
      <Card>
        <p className="section-eyebrow">Mensagens</p>
        <p className="hint mt-3">
          Nada por aqui ainda. Confirmações de horário, lembretes e avisos do
          leva-e-traz aparecem nesta lista assim que forem enviados.
        </p>
      </Card>
    )
  }
...
function Linha({ mensagem, timezone }: { mensagem: PortalMessage; timezone: string }) {
  return (
    <li className="border-line flex flex-col gap-1 border-b py-3 last:border-b-0">
      <p className="hint">
        {data(mensagem.sentAt, timezone)} · {CANAL[mensagem.channel]} ·{' '}
        {CATEGORIA[mensagem.category]}
      </p>

      {mensagem.subject && <p className="text-sm font-medium">{mensagem.subject}</p>}

      {/*
        `whitespace-pre-line` porque o corpo foi escrito com quebras de linha e é assim
        que ele chegou no WhatsApp. Colapsar tudo numa parágrafo só faria a mensagem
        relida parecer outra mensagem.
      */}
      <p className="text-muted text-sm whitespace-pre-line">{mensagem.body}</p>
    </li>
  )
}
